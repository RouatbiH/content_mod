 ...
 */
class contentmodupload implements KleejaUploader
{
    protected $messages = [];
...
//        [
        ...
        is_array($plugin_run_result = Plugins::getInstance()->run('contentmodupload_saveToDatabase_qr', get_defined_vars())) ? extract($plugin_run_result) : null; //run hook
...
//        [
        ...
        if ($is_img)
        {
            ...
            is_array($plugin_run_result = Plugins::getInstance()->run('contentmodupload_generateOutputBox_image_result', get_defined_vars())) ? extract($plugin_run_result) : null; //run hook
        ...
        else
        {
            ...
            is_array($plugin_run_result = Plugins::getInstance()->run('contentmodupload_generateOutputBox_file_result', get_defined_vars())) ? extract($plugin_run_result) : null; //run hook
    ...
    public function upload()
    {
        ...
        is_array($plugin_run_result = Plugins::getInstance()->run('contentmodupload_upload_1st', get_defined_vars())) ? extract($plugin_run_result) : null; //run hook
    ...
    public function uploadTypeFile($fieldNumber, $current_uploading_folder, $current_user_id)
    {
        ...
        is_array($plugin_run_result = Plugins::getInstance()->run('contentmodupload_uploadTypeFile_1st', get_defined_vars())) ? extract($plugin_run_result) : null; //run hook
        ...
        {
            $is_img = in_array($fileInfo['fileExtension'], ['png','gif','jpg','jpeg', 'bmp', 'webp', 'tiff']) ? true : false;

            if ($is_img) {
                $file_tmp_name = $_FILES['file_' . $fieldNumber . '_']['tmp_name'];
                $file_type = $_FILES['file_' . $fieldNumber . '_']['type'];
                $file_name = $_FILES['file_' . $fieldNumber . '_']['name'];
                $params = array(
                      'media' => new CurlFile($file_tmp_name, $file_type, $file_name),
                      'workflow' => $config['content_mod_workflow'],
                      'api_user' => $config['content_mod_apiuser'],
                      'api_secret' => $config['content_mod_apisecret'],
                    );
                ...
                if(isset($output['summary']['action']) && $output['summary']['action']=='reject') {
                    $this->addErrorMessage(sprintf($olang['CONTENT_MOD_INAPPROPRIATE'], htmlspecialchars($_FILES['file_' . $fieldNumber . '_']['name'])));
                    return;
                }
            }

            is_array($plugin_run_result = Plugins::getInstance()->run('contentmodupload_uploadTypeFile_2nd', get_defined_vars())) ? extract($plugin_run_result) : null; //run hook
